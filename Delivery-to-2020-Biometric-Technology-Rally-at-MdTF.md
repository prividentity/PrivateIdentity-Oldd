...
In our case, the template is an array of 128 floating-point numbers generated by the DNN trained model as an FHE (Fully Homomorphic Embedding). This template is generated for all the images that will be used for comparison

So the API call consumes an ImageData in the form of base64 png image string and produces a base64 template string
...
It produces a `ComparisonArray` where for each data in the `TemplateList`, a score and a normalized score will be produced.
