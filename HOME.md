...
We threw the old algorithms into the trash and started fresh with a clean-sheet design. Cloud Biometric MFA took three years and seventeen patents to build. This all-new AI/ML/Fully Homomorphic Encryption (FHE) recognition algorithm features absolute accuracy, 300ms response time, 4kB FHE payloads, unlimited users and full privacy. Now, for the first time, your private identity is no longer tied to a device, username, password, token or shared secret.
